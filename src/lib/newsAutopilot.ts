...
export function completeNewsDraftLength<T extends Pick<ComposedNews, 'content'>>(draft: T, candidate: Pick<NewsCandidate, 'sourceName' | 'sourcePublishedAt' | 'title' | 'summary'>, site = defaultNewsSite()): T {
  ...
  const additions = [
    ...
    'A dated, source-linked briefing is most helpful when it preserves both relevance and restraint. Relevance comes from connecting the reported topic to the kinds of decisions that marine and water-sports organizations routinely face. Restraint comes from declining to claim that the report settles a local question without local evidence. This balance allows readers to notice a potentially useful development, discuss it with the appropriate people, and decide whether a more specific review is warranted. It also keeps the editorial record clear: the reported facts belong to the cited source, while the surrounding operational considerations are presented as questions and context rather than as proof of a result.'
  ];
